...
void f(X &L, pair<int, int> x) {
  if (x.fi == -1) return;
  FOR(i, S) if (L[i].fi == x.fi) return L[i].se += x.se, void();
  FOR(i, S) if (L[i].fi == -1) return L[i] = x, void();
  int d = x.se;
  FOR(i, S) if (L[i].fi != -1) chmin(d, L[i].se);
  x.se -= d;
  FOR(i, S) if ((L[i].se -= d) <= 0) L[i] = {-1, 0};
  if (x.se > 0) FOR(i, S) if (L[i].fi == -1) return L[i] = x, void();
}
X op(X L, X R) {
  FOR(i, S) f(L, R[i]);
  return L;
