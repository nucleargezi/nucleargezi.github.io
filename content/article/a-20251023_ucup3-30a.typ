...
#show: main.with(
  title: "Record The 3rd Universal Cup. Stage 30: Northern",
  desc: [UCUP 3-30 部分题解],
  date: "2025-10-23",
  tags: ("icpc",),
  category: "ICPC",
...
=== 简要题意
在一个 $N * N$ （$N<=10^9$）的空间内 有一个随机的特殊整点，需要找到它，每次可以询问一个整点 $O$ ，得到空间中射线 $arrow(O A)$ 所在的半平面内的随机一个点 $A$ ，在90次询问中找出这个点，询问和回答为同一个点即为找到。

=== Solution
对于点可能在的范围，将其看作一个凸多边形，由于每次询问可以得到一个半平面的限制，每次询问重心可以将可选范围切割掉一半，这似乎是一个很好想到的解决方案，操作次数也绰绰有余（可以看作在 $x, y$ 两个维度的二分，操作最劣应该是 $30+30$ 次）。

我并不打算卖关子，这种做法在一些情况，比如多边形变成 边长为 $2$ 的矩形 时会出问题，在 $N=2$ 的 case 就会直接挂掉，由于询问和回答的是整点，这时询问的重心会是一个角，无论回答的是哪个角，直接切割后得到的新凸多边形会和原来的一样，于是会不断询问同一个点。
...
P s = polygon.center();
ll x = llround(s.x + (rng() & 1 ? eps : -eps)),
   y = llround(s.y + (rng() & 1 ? eps : -eps));
```
...
vc<P> convex_cut(P s, P t) const {
  ...
  for (int i = 0; i < N; ++i) {
    ...
    if (p1_ins)
      res.ep(p1);
    if (p1_ins != p2_ins)
      res.ep(l.cross_point(line(p1, p2)).se);
  }
  return res;
}
```
